...
> **Paradigma:** Classical (HOG + SVM) vs Deep Learning (YOLOv8 Multi-Class Detection + CNN Classifier)  
...
Il repository è stato migrato completamente dal progetto precedente (Hand Gesture Recognition — MediaPipe + SVM e YOLOv8 single-class) alla nuova struttura Traffic Monitoring. Tutti i moduli core sono implementati e testati.
...
| Evaluation compare | `src/evaluation/compare.py` | ✅ DONE | Confronto side-by-side Classical vs DL pipeline |
| Visualization base | `src/utils/visualization.py` | ✅ DONE | Draw bbox multi-class, contatori, linea |
| Config loader | `src/config.py` | ✅ DONE | Config centralizzato con path traffic, classi, iperparametri HOG |
| Classical HOG detector | `src/classical/hog_detector.py` | ✅ DONE | HOGDescriptor + sliding window |
| Classical vehicle classifier | `src/classical/vehicle_classifier.py` | ✅ DONE | SVM wrapper per classificazione crop |
| Classical train | `src/classical/train.py` | ✅ DONE | Estrazione HOG features + training SVM |
...
| Webcam pipeline | `src/webcam/pipeline.py` | ✅ DONE | Loop live con switch CLASSICAL / DEEP + conteggio |
| Tests IoU/NMS | `tests/test_iou.py`, `tests/test_nms.py` | ✅ DONE | 10 test passano |
| Tests HOG/classifier | `tests/test_hog_detector.py`, `tests/test_classifier.py` | ✅ DONE | 4 test passano |
| Tests pipeline | `tests/test_pipeline.py` | ✅ DONE | 13 test passano |
...
Obiettivo: Eliminare codice legacy hand-gesture e creare la struttura per traffic monitoring.

- [x] Rimuovere `src/classical/hand_detector.py`, `feature_extractor.py`, `gesture_classifier.py`
- [x] Rimuovere `src/deep_learning/gesture_net.py`
- [x] Creare `src/classical/hog_detector.py`, `vehicle_classifier.py`, `train.py`
...
- [x] Scrivere `src/config.py` con classi VisDrone, parametri HOG, path modelli traffic
...
### FASE B — Classical Pipeline: HOG + SVM ✅ COMPLETATA
Obiettivo: Pipeline classica funzionante end-to-end.

- [x] `src/classical/hog_detector.py` — HOGDescriptor wrapper (default people detector + custom)
- [x] `src/classical/vehicle_classifier.py` — SVM wrapper con StandardScaler
- [x] `src/classical/train.py` — Estrazione HOG features da crop + training
- [x] `scripts/train_svm.py` — Addestra HOG+SVM + salva modello
- [x] `tests/test_hog_detector.py` — Test HOG descriptor e detection
- [x] `tests/test_classifier.py` — Test SVM

### FASE C — DL Pipeline: YOLOv8 + CNN ✅ COMPLETATA
...
- [x] `scripts/build_classifier_dataset.py` — Estrae crop 224×224 per CNN/SVM
...
### FASE D — Integration, Webcam & Benchmark ✅ COMPLETATA
...
- [x] `src/webcam/pipeline.py` — Loop live con switch `c`/`d` + linea di conteggio + tracker
- [x] `scripts/run_webcam.py` — Entrypoint CLI
- [x] `src/evaluation/compare.py` — Confronto Classical vs DL su traffic
- [x] `tests/test_pipeline.py` — Test end-to-end per nuova pipeline

### FASE E — Documentation & Polish 🔄 IN CORSO
...
| `requirements.txt` / `environment.yml` | Root | Obbligatorio | ✅ Aggiornati con opencv, ultralytics, torch |
| Dataset script (download & prep) | `scripts/download_visdrone.py` | Obbligatorio | ✅ Creato e funzionante |
| Modello Classico (SVM) | `models/hog_svm.pkl` | Obbligatorio | ⚠️ Da addestrare |
...
| **HOG+SVM invece di MediaPipe** | MediaPipe è specifico per mani. HOG+SVM è lo standard CV classico per object detection (pedoni/veicoli). |
